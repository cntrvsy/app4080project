<script lang="ts">
    ...
    import del from '$lib/assets/del.svg';
    ...
    <section>
        <div class="container mx-auto flex px-5 py-8 items-center justify-center flex-col">
          <img class="lg:w-2/6 md:w-3/6 w-5/6 mb-10 object-cover object-center rounded" alt="hero" src="{del}">
          <div class="text-center lg:w-2/3 w-full">
            <h1 class="title-font sm:text-4xl text-3xl mb-4 font-medium">Hey, {data.session.user.email}</h1>
            <p class="leading-relaxed mb-8">
                Delete a project from your portfolio here, select the project.<br>
                You must first select the project, then perform the delete operation.
            </p>
            <div class="flex justify-center">
                <!-- update section -->
                <div class="card p-4 w-full text-token space-y-4">
                    <form on:submit={()=>deleteProject()}>
                        <button type="submit" class="btn variant-filled-primary">Delete</button>
                    </form>
                </div>
            </div>
          </div>
